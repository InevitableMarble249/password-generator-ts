...
async function fetchRandomNoun(): Promise<{ word: string; definition: string }> {
  ...
  try {
    const response = await axios.get<string[]>(NOUNS_URL);
    console.log('Noun data received:', response.data.slice(0, 5), '...');
    const nouns = response.data;
    const randomNoun = getRandomElement(nouns) as string;
    console.log('Selected random noun:', randomNoun);
...
async function fetchRandomAdjective(): Promise<{ word: string; definition: string }> {
  ...
  try {
    const response = await axios.get<string[]>(ADJECTIVES_URL);
    console.log('Adjective data received:', response.data.slice(0, 5), '...');
    const adjectives = response.data;
    const randomAdjective = getRandomElement(adjectives) as string;
    console.log('Selected random adjective:', randomAdjective);
...
app.get('/generate', async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(500).json({ 
      error: 'Failed to generate password. Error fetching dictionary data.',
